...
Extends [React Native View](https://reactnative.dev/docs/view#props) props. Auto-sets `showSeparator` on child `InlineListItem`s (all but the last).
...
<TypeTable
  type={{
    showSeparator: {
      description: "Shows a hairline divider below the item. Inside `InlineList`, defaults to true for all but the last item.",
      type: "boolean",
      default: "false",
    },
    variant: {
