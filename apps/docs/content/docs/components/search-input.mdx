---
title: Search Input
description: An input component that allows users to search for content.
---

<ComponentPreview name="search-input" type="png" />

## Installation

<Tabs defaultValue="cli">

<TabsList>
  <TabsTrigger value="cli">CLI</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```package-install
npx shadcn@latest add @tetra-ui/search-input
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="search-input" title="components/ui/search-input.tsx" />

</Steps>

</TabsContent>

</Tabs>


## Usage

```tsx
import { SearchInput } from "@/components/ui/search-input";
```

```tsx
<SearchInput placeholder="Search..." />
```

A search icon is rendered automatically on the leading edge.

## With addon

Add trailing content such as a clear button:

```tsx
import { InputAddon, InputAddonButton, InputAddonButtonIcon } from "@/components/ui/input";
import { XIcon } from "@/components/ui/icons";

<SearchInput placeholder="Search..." value={query} onChangeText={setQuery}>
  <InputAddon align="inline-end">
    <InputAddonButton size="icon" variant="ghost" onPress={() => setQuery("")}>
      <InputAddonButtonIcon>
        <XIcon />
      </InputAddonButtonIcon>
    </InputAddonButton>
  </InputAddon>
</SearchInput>
```

## Disabled

```tsx
<SearchInput disabled placeholder="Search..." />
```

## API Reference

### SearchInput

Extends [`Input`](/docs/components/input) props. Renders a leading search icon. Defaults `accessibilityRole` to `"search"`.

<TypeTable
  type={{
    disabled: {
      description: "Disables the field and pressable wrapper.",
      type: "boolean",
    },
    children: {
      description: "Additional `InputAddon` children for leading or trailing content.",
      type: "InputAddonChildren",
    },
  }}
/>


