...
Exported icons (for example `BadgeCheckIcon`, `ChevronRightIcon`) are Lucide icons wrapped with Uniwind. They accept [Lucide React Native](https://lucide.dev/guide/packages/lucide-react-native) props plus `className`, and default to `size-6 text-foreground`.
