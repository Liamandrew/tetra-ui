...
Extends [`InputPressable`](/docs/components/input) props. Displays `value` or `placeholder` as text (not an editable TextInput). A trailing chevron is rendered when no end addon is provided.
